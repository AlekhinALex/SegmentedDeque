...
T MainController::parseValue(const QVariant &value, ValueType valueType) const {
    if constexpr (std::is_same_v<T, int>) {
        if (valueType != ValueType::Int) throw std::invalid_argument("Type mismatch: expected int");
        bool ok;
        int result = value.toInt(&ok);
        if (!ok) throw std::invalid_argument("Invalid integer value");
        return result;
    } else if constexpr (std::is_same_v<T, double>) {
        if (valueType != ValueType::Double) throw std::invalid_argument("Type mismatch: expected double");
        bool ok;
        double result = value.toDouble(&ok);
        if (!ok) throw std::invalid_argument("Invalid double value");
        return result;
    } else if constexpr (std::is_same_v<T, std::string>) {
        if (valueType != ValueType::String) throw std::invalid_argument("Type mismatch: expected string");
        return value.toString().toStdString();
    } else if constexpr (std::is_same_v<T, Person>) {
        if (valueType != ValueType::Person) throw std::invalid_argument("Type mismatch: expected Person");
        ...
        if (!ok) throw std::invalid_argument("Invalid age in Person");
        return Person(name.toStdString(), age);
    } else if constexpr (std::is_same_v<T, Complex>) {
        if (valueType != ValueType::Complex) throw std::invalid_argument("Type mismatch: expected Complex");
        ...
        if (!ok) throw std::invalid_argument("Invalid real part in Complex");
        double imag = match.captured(3).toDouble(&ok);
        if (!ok) throw std::invalid_argument("Invalid imaginary part in Complex");
...
void MainController::sortImmutableSequence(const std::string &originalName, const std::string &newName, bool ascending) {
    ...
    try {
        ...
        switch (original.valueType) {
            case ValueType::Int: {
                auto *originalSeq = getSequence<int>(originalName);
                auto *newSeq = ascending ?
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end()) :
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end(), std::greater<int>());
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<int>>(newSeq);
                break;
            }
            case ValueType::Double: {
                ...
                auto *newSeq = ascending ?
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end()) :
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end(), std::greater<double>());
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<double>>(newSeq);
                break;
            }
            case ValueType::String: {
                ...
                auto *newSeq = ascending ?
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end()) :
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end(), std::greater<std::string>());
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<std::string>>(newSeq);
                break;
            }
            case ValueType::Person: {
                ...
                auto *newSeq = ascending ?
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end()) :
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end(), std::greater<Person>());
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<Person>>(newSeq);
                break;
            }
            case ValueType::Complex: {
                ...
                auto *newSeq = ascending ?
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end()) :
                    originalSeq->sortImmutable(originalSeq->begin(), originalSeq->end(), std::greater<Complex>());
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<Complex>>(newSeq);
                break;
            }
        }
...
void MainController::where(const std::string &name, const std::string &newName, const QString &condition) {
    ...
    try {
        ...
        switch (data.valueType) {
            case ValueType::Int: {
                auto pred = condition == "positive" ? [](int x) { return x > 0; } :
                            condition == "even" ? [](int x) { return x % 2 == 0; } :
            ...
            case ValueType::Double: {
                auto pred = condition == "positive" ? [](double x) { return x > 0.0; } :
                            condition == "greater_one" ? [](double x) { return x > 1.0; } :
            ...
            case ValueType::String: {
                auto pred = condition == "non_empty" ? [](const std::string &s) { return !s.empty(); } :
            ...
            case ValueType::Person: {
                auto pred = condition == "adult" ? [](const Person &p) { return p.getAge() >= 18; } :
            ...
            case ValueType::Complex: {
                auto pred = condition == "non_zero" ? [](const Complex &c) { return c.magnitude() > 0.0; } :
    ...
    try {
        ...
        switch (data.valueType) {
            ...
            case ValueType::String: {
                ...
                return valueToQVariant(getSequence<std::string>(name)->reduce([](const std::string &a, const std::string &b) { return a + b; }, init));
            }
            case ValueType::Person:
                throw std::runtime_error("Reduce not supported for Person type");
            case ValueType::Complex: {
...
void MainController::applyUnaryOp(const std::string &name, const std::string &newName, const QString &operation) {
    ...
    try {
        ...
        switch (data.valueType) {
            case ValueType::Int: {
                auto op = operation == "square" ? [](int x) { return x * x; } :
                          operation == "increment" ? [](int x) { return x + 1; } :
                          throw std::invalid_argument("Invalid operation for int");
                auto *originalSeq = getSequence<int>(name);
                auto *newSeq = new SegmentedDeque<int>();
                originalSeq->apply(originalSeq->begin(), originalSeq->end(), newSeq->begin(), op);
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<int>>(newSeq);
                break;
            }
            case ValueType::Double: {
                auto op = operation == "square" ? [](double x) { return x * x; } :
                          operation == "increment" ? [](double x) { return x + 1.0; } :
                          throw std::invalid_argument("Invalid operation for double");
                auto *originalSeq = getSequence<double>(name);
                auto *newSeq = new SegmentedDeque<double>();
                originalSeq->apply(originalSeq->begin(), originalSeq->end(), newSeq->begin(), op);
                newSequenceData.sequence = std::unique_ptr<SegmentedDeque<double>>(newSeq);
                break;
            }
